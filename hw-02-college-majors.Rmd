...
4.  Based on the shape of the histogram you created in the previous exercise, determine which of these summary statistics is useful for describing the distribution.
    ...
    Based on this data, I think the median is most important.  The statistics also concluded what we found in the graph and supported it.  The spread of the data had large portions in the middle wiht smaller pourtions around it, this shows that there is a pattern in the data.  
...
5.  Plot the distribution of `median` income using a histogram, faceted by `major_category`.
    ...
    ```{r fig,height=2}
ggplot(data = college_recent_grads, major_category, mapping = aes(x = median)) +
  geom_histogram(binwidth = 1000)
```
...
```{r}
college_recent_grads %>%
  filter(stem_median <= median major_type == "stem",
    median < 36000
  )
```
🧶 ✅ ⬆️ Knit, *commit, and push your changes to GitHub with an appropriate commit message. Make sure to commit and push all changed files so that your Git pane is cleared up afterwards.*
...
9.  Create a scatterplot of median income vs. proportion of women in that major, coloured by whether the major is in a STEM field or not. Describe the association between these three variables.
 ...
 ```{r fig,height=2}
ggplot(data = college_recent_grads, women, mapping = aes(x = median_income, y= women)) +
  scatterplot(binwidth = 1000)
```
